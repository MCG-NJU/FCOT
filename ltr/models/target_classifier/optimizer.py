...
class DiMPSteepestDescentGN(nn.Module):
    ...
    def forward(self, weights, feat, bb, sample_weight=None, num_iter=None, compute_losses=True):
        ...
        # print("bb: {}, center: {}".format(bb, center))
        dist_map = self.distance_map(center, output_sz)
        # print("dist map shape:{}".format(dist_map.shape))
        ...
        target_mask = self.target_mask_predictor(dist_map).view(num_images, num_sequences, *dist_map.shape[-2:])  # m
        spatial_weight = self.spatial_weight_predictor(dist_map).view(num_images, num_sequences, *dist_map.shape[-2:]) # v
        ...
        for i in range(num_iter):
            ...
            # print("feat shape: {}, weights shape: {}".format(feat.shape, weights.shape))
            # feat shape: [num_images, num_sequences, 256, 72, 72]
            # weights shape: [num_sequences, 256, 4, 4]
            ...
            residuals = sample_weight * (scores_act - label_map)   # (num_images, num_sequences, 73, 73)
            # print("scores shape: {}, scores_act shape: {}, residuals shape: {}".format(scores.shape, scores_act.shape, residuals.shape))

            if compute_losses:
                losses.append((residuals**2).mean())

            # Compute gradient
            residuals_mapped = score_mask * (sample_weight * residuals)    # q_c * r_(s,c)
            weights_grad = filter_layer.apply_feat_transpose(feat, residuals_mapped, filter_sz, training=self.training) + \
                          reg_weight * weights
            # print("weights_grad shape: {}".format(weights_grad.shape))  # [num_sequences, 256, 4, 4]
            ...
            alpha = alpha_num / alpha_den   #(num_sequences)
            # if i == num_iter - 1:
            #     print("clf alpha: {}".format(alpha))
            # print("alpha_num shape: {}, alpha_den shape: {}, alpha shape: {}".format(alpha_num, alpha_den, alpha))
